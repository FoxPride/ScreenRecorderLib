	...
	if (lastElt == firstElt + 1)
	{
		if (buildForDither)
		{
			...
			if (firstElt == 1)
			{
				...
				{
					r = static_cast<uint32_t>(GifIMin(static_cast<int32_t>(r), image[ii * 4 + 0]));
					g = static_cast<uint32_t>(GifIMin(static_cast<int32_t>(g), image[ii * 4 + 1]));
					b = static_cast<uint32_t>(GifIMin(static_cast<int32_t>(b), image[ii * 4 + 2]));
				}
			...
			if (firstElt == (1 << pal->bitDepth) - 1)
			{
				...
				for (int ii = 0; ii < numPixels; ++ii)
				{
					r = static_cast<uint32_t>(GifIMax(static_cast<int32_t>(r), image[ii * 4 + 0]));
					g = static_cast<uint32_t>(GifIMax(static_cast<int32_t>(g), image[ii * 4 + 1]));
					b = static_cast<uint32_t>(GifIMax(static_cast<int32_t>(b), image[ii * 4 + 2]));
				}
...
void GifDitherImage(const uint8_t* lastFrame, const uint8_t* nextFrame, uint8_t* outFrame, uint32_t width, uint32_t height, GifPalette* pPal)
{
	...
	for (int ii = 0; ii < numPixels * 4; ++ii)
	{
		...
		int32_t pix16 = static_cast<int32_t>(pix) * 256;
	...
	for (uint32_t yy = 0; yy < height; ++yy)
	{
		for (uint32_t xx = 0; xx < width; ++xx)
		{
			...
			int32_t r_err = nextPix[0] - static_cast<int32_t>(pPal->r[bestInd]) * 256;
			int32_t g_err = nextPix[1] - static_cast<int32_t>(pPal->g[bestInd]) * 256;
			int32_t b_err = nextPix[2] - static_cast<int32_t>(pPal->b[bestInd]) * 256;
...
bool GifBegin(GifWriter* writer, const wchar_t* filename, uint32_t width, uint32_t height, uint32_t delay, int32_t bitDepth, bool dither)
{
	const _bstr_t path(filename);

	(void)bitDepth; (void)dither; // Mute "Unused argument" warnings
#if defined(_MSC_VER) && (_MSC_VER >= 1400)
	writer->f = nullptr;
	fopen_s(&writer->f, path, "wb");
#else
